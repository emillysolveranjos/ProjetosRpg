...
import type { CombatantState, TokenView, Viewer } from "../domain/types";
...
import { ConditionLibrary } from "./RuleEditors";
...
export function App({ gateway }: { gateway?: OwlbearGateway }) {
  ...
  return <main className="app-shell compact-app">
    ...
    <footer className="footer-bar"><span role="status">{busy ? "Salvando…" : notice && !previewing ? "✓ Salvo" : online ? "Sincronizado" : "Consulta"}</span>{gm && <button className="text-button" onClick={() => setDialog("library")}>Condições</button>}<button className="text-button" onClick={() => setDialog("history")}>Histórico <b>{history.length}</b></button></footer>
