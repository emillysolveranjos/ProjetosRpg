...
import type { CombatantState, ConditionDefinition, DamageComponent, DamageReduction, DamageTypeDefinition, DefensePreset, RulebearSceneState, Trigger } from "../domain/types";
...
export const blankDamageComponent = (): DamageComponent => ({ expression: "", damageTypeIds: [], ignoreImmunity: false });

export function DamageComponentRows({ types, components, onChange, disabled = false }: {
  types: DamageTypeDefinition[]; components: DamageComponent[]; onChange(value: DamageComponent[]): void; disabled?: boolean;
}) {
  const update = (index: number, patch: Partial<DamageComponent>) => onChange(components.map((component, position) => position === index ? { ...component, ...patch } : component));
  return <section className="damage-components" aria-label="Componentes de dano">
    <header><strong>Componentes do ataque</strong><small>Cada linha resolve seu próprio tipo, imunidade e RD.</small></header>
    {components.map((component, index) => {
      const hybrid = component.damageTypeIds.length > 1;
      const selectedType = hybrid ? "__hybrid__" : component.damageTypeIds[0] ?? "";
      const hybridNames = component.damageTypeIds.map((typeId) => types.find((type) => type.id === typeId)?.name ?? "Tipo removido").join(" + ");
      return <fieldset className="damage-component" key={index} disabled={disabled}><legend>Componente {index + 1}</legend>
        <div className="component-grid"><label>Dano<input value={component.expression} onChange={(event) => update(index, { expression: event.target.value })} placeholder="2d6+3" /></label><label>Tipo<select value={selectedType} onChange={(event) => update(index, { damageTypeIds: event.target.value ? [event.target.value] : [] })}>{hybrid && <option value="__hybrid__">Híbrido: {hybridNames}</option>}<option value="">Sem tipo</option>{types.map((type) => <option key={type.id} value={type.id}>{type.name}</option>)}</select></label></div>
        {hybrid && <small className="field-help">Componente antigo híbrido. Escolha um tipo para convertê-lo ou mantenha como está.</small>}
        <div className="component-options"><label className="check"><input type="checkbox" checked={component.ignoreImmunity} onChange={(event) => update(index, { ignoreImmunity: event.target.checked })} /> Ignorar imunidade</label><label>Ignorar RD<input value={component.ignoreReductionExpression ?? ""} onChange={(event) => update(index, { ignoreReductionExpression: event.target.value || undefined })} placeholder="0, 1d6+2…" /></label></div>
        {components.length > 1 && <button type="button" className="button danger subtle" onClick={() => onChange(components.filter((_, position) => position !== index))}>Remover componente</button>}
      </fieldset>;
    })}
    <button type="button" className="button" disabled={disabled || components.length >= 12} onClick={() => onChange([...components, blankDamageComponent()])}>＋ Adicionar componente</button>
  </section>;
}
...
export function ReductionEditor({ state, combatant, onDone }: { state: RulebearSceneState; combatant: CombatantState; onDone(): void }) {
  ...
  return <form className="reduction-editor form-stack" onSubmit={save}>
    <section className="preset-apply"><div><h3>Adicionar um preset</h3><p className="muted">Uma cópia independente será adicionada ao token.</p></div><div className="inline-action"><select aria-label="Preset de defesa" value={presetId} onChange={(event) => setPresetId(event.target.value)}><option value="">Escolher preset…</option>{state.defensePresets.map((preset) => <option value={preset.id} key={preset.id}>{preset.name} · {preset.kind === "IMMUNITY" ? "Imune" : `RD −${preset.amount}`}</option>)}</select><button type="button" className="button primary" disabled={readonly || !presetId} onClick={() => void useAppStore.getState().command({ type: "applyDefensePreset", tokenId: combatant.tokenId, presetId }).then((saved) => { if (saved) onDone(); })}>Adicionar ao token</button></div></section>
    ...
    {rows.map((row, index) => <fieldset className="defense-card" key={row.id}><legend>{row.label.trim() || `Defesa ${index + 1}`}</legend>
      <div className="form-grid"><label>Nome<input aria-label="Nome da defesa" value={row.label} onChange={(event) => update(index, { label: event.target.value })} placeholder="Armadura" /></label><label>Tipo de defesa<select value={row.kind} onChange={(event) => update(index, { kind: event.target.value as DamageReduction["kind"], amount: event.target.value === "IMMUNITY" ? 0 : row.amount })}><option value="REDUCTION">Redução fixa</option><option value="IMMUNITY">Imunidade</option></select></label>{row.kind === "REDUCTION" && <label>Valor da RD<input aria-label="Valor da redução" inputMode="numeric" value={row.amount} onChange={(event) => update(index, { amount: Number(event.target.value) })} /></label>}</div>
...
function ConditionSection({ state }: { state: RulebearSceneState }) {
  ...
  const [components, setComponents] = useState<DamageComponent[]>([{ ...blankDamageComponent(), expression: "1" }]);
  function reset() { setDraft(blank()); setHasEffect(false); setDuration(""); setExpression("1"); setComponents([{ ...blankDamageComponent(), expression: "1" }]); }
  function edit(definition: ConditionDefinition) {
    ...
    if (effect) {
      setKind(effect.kind); setTrigger(effect.trigger); setExpression(effect.expression);
      const damageEffects = definition.effects.filter((item) => item.kind === "DAMAGE" && item.trigger === effect.trigger);
      setComponents(damageEffects.length ? damageEffects.map((item) => ({ expression: item.expression, damageTypeIds: item.damageTypeIds, ignoreImmunity: item.ignoreImmunity, ...(item.ignoreReductionExpression ? { ignoreReductionExpression: item.ignoreReductionExpression } : {}) })) : [{ ...blankDamageComponent(), expression: "1" }]);
    } else setComponents([{ ...blankDamageComponent(), expression: "1" }]);
    setDuration(definition.duration ? String(definition.duration.ticks) : "");
  }
  function save(event: FormEvent) {
    event.preventDefault();
    const first = draft.effects[0];
    const group = draft.effects.filter((effect) => effect.kind === first?.kind && effect.trigger === first?.trigger);
    const untouched = draft.effects.filter((effect) => !group.includes(effect));
    const changed = !hasEffect ? [] : kind === "HEAL"
      ? [{ id: group[0]?.id ?? id(), kind, trigger, expression, damageTypeIds: [], multiplyByStacks: group[0]?.multiplyByStacks ?? true, ignoreImmunity: false }]
      : components.map((component, index) => ({ id: group[index]?.id ?? id(), kind: "DAMAGE" as const, trigger, expression: component.expression, damageTypeIds: component.damageTypeIds, multiplyByStacks: group[index]?.multiplyByStacks ?? true, ignoreImmunity: component.ignoreImmunity, ...(component.ignoreReductionExpression?.trim() ? { ignoreReductionExpression: component.ignoreReductionExpression.trim() } : {}) }));
    const effects = [...changed, ...untouched];
    const definition: ConditionDefinition = { ...draft, maximumStacks: Number(draft.maximumStacks), ...(duration ? { duration: { ticks: Number(duration), decrementOn: trigger } } : {}), effects };
...
function DefensePresetSection({ state }: { state: RulebearSceneState }) {
  const readonly = useAppStore((store) => !store.online || store.busy);
  const blank = (): DefensePreset => ({ id: id(), name: "", kind: "REDUCTION", amount: 0, damageTypeIds: [] });
